...
class PainelUser extends Component {
    ...
    public $mediasInovacao = [];
    public $mediasOrientacao = [];
    public $mediasMaturidade = [];
    ...
    public function mount(Request $request) {
        ...
        $this->empresa = User::find($id);
        $this->questoes = Questao::all();

        $this->respostas = Resposta::select('questao_id', 'resultado')
                            ->where('user_id', $this->empresa->id)
                            ->get();
    ...
    public function verificaMaturidade() {  
        ...
        */
        ...
        foreach ($relacionamentosPorSegmento as $key => $segmento) {
            $this->mediasMaturidade[] = number_format( array_sum($segmento) / count($segmento), 2, '.', '');
        }
    }
    public function resgataMedias() {
    ...
    public function deletUser(){
        dd('Deletando Empresa','Nome: '.$this->empresa->name);
    }

    public function render() {
        return view('livewire.painel-user');
    }
}
